...
func runShowConfNode(_ *cobra.Command, args []string) error {
	...
	t, err := mesh.NewTopology(nodes, peers, opts.granularity, hostname, "node", opts.port, []byte{}, subnet, nodes[hostname].PersistentKeepalive)
	if err != nil {
...
func runShowConfPeer(_ *cobra.Command, args []string) error {
	...
	t, err := mesh.NewTopology(nodes, peers, opts.granularity, hostname, "node", mesh.DefaultKiloPort, []byte{}, subnet, peers[peer].PersistentKeepalive)
	if err != nil {
		return fmt.Errorf("failed to create topology: %v", err)
	}
	if !showConfOpts.asPeer {
